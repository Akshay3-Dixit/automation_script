...
#         print(os.getcwd())
    ...
    def audio_metadata(self, stream_name, audio_stream_channel_map, audio_channels, audio_indexes, audio_codec_name ):
        ...
        if stream_name.bits_per_sample == '0':
            audio_codec_name = "pcm_s" + "16" + "le"
        else:
            audio_codec_name = "pcm_s" + stream_name.bits_per_sample + "le"
        profile = profile[:-1]
    ...
    def write_output(self):
        try:
            ...
            data = pd.DataFrame(self.file_info, columns=["datetime","Filename", "Profile", "Duration", "Status"])
            data.to_csv('out.csv', index = False)
